...
Campaign state: release candidate after a material user-directed resume correction. The campaign returns to complete only when `deployment-status.json` identifies the corrected publication source and records `live_audit: passed`.
...
- Durable regression: source, generated-PDF, and exact-live audits must return zero matches for `Food Safety Management Certification`.
...
- Credential-removal quality run `29854417415` passed source regression, the five-viewport browser matrix, document geometry, generation of all five retained PDFs, exact two-page resume pagination, dual-parser page-count agreement, and zero PDF-text matches for the removed certification.
